...
public class UserController {
    ...
    @Operation(
            ...
            description = """
                ...
                프론트에서는 응답값을 다음과 같이 사용할 수 있습니다.

                - emailVerified가 false이면 학교 이메일 인증 화면으로 이동합니다.
                - emailVerified가 true이고 profileCompleted가 false이면 온보딩 프로필 입력 화면으로 이동합니다.
                - emailVerified와 profileCompleted가 모두 true이면 메인 화면 또는 마이페이지를 표시합니다.

                departmentName은 화면 표시용으로 사용하고,
                departmentId는 프로필 수정 화면에서 기존 학과 선택값을 표시할 때 사용할 수 있습니다.

                campus는 학번을 기준으로 백엔드가 분류한 캠퍼스 정보입니다.
                preferredRegion은 사용자가 선택한 시·군·구 단위 선호 활동 지역입니다.

                취미 태그, 기술 태그, 가용 일정은 각각의 전용 API를 통해 조회합니다.
    ...
    public ResponseEntity<ApiResponse<NicknameCheckResponse>> checkNickname(
        ...
        NicknameCheckResponse response =
                new NicknameCheckResponse(available, nickname);
