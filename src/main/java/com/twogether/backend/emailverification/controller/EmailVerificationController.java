...
public class EmailVerificationController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    Supabase 소셜 로그인 후 호출하는 API이며,
                    access token을 Authorization 헤더에 전달해야 합니다.

                    실제 구현에서는 @mju.ac.kr 이메일 형식을 확인하고
                    해당 이메일로 인증번호를 발송합니다.

                    현재 Swagger 명세 단계에서는 실제 이메일을 발송하지 않고
                    성공 응답만 반환합니다.
                    """
    )
    @PostMapping
    ...
    @Operation(
            ...
            description = """
                    ...
                    실제 구현에서는 이메일에 발급된 인증번호와
                    사용자가 입력한 인증번호가 일치하는지 확인합니다.

                    인증에 성공하면 해당 사용자의 학교 이메일 인증 상태를
                    완료로 변경합니다.
